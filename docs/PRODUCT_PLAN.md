...
| 26 | Wrong-answer-key remediation | **Decided (owner, 31 Aug 2026)** | **Recompute + notify.** When an answer key is found to be wrong after learners have taken the paper: fix the key, recompute every affected attempt's score and weakness report, and show affected learners a clear notice explaining what was corrected and how their score changed. Our error is never covered by the no-refund policy silence — learners are always told. |
| 27 | Final public wording of the product claim | **Decided (owner, 31 Aug 2026)** | **"Unlimited retakes from a growing bank of verified questions."** Canonical learner-facing promise: *"Full UPSC Prelims mock series — one payment (₹249), unlimited retakes from a growing bank of verified questions."* No fixed paper count is ever promised; while the bank is small the UI keeps the honest-labelling disclosure that papers share questions (per #18/#25). Marketing copy, course titles/descriptions, and UI labels follow this wording. |
| 28 | Does GS Paper I launch in v1? | **Decided (owner, 31 Aug 2026)** | **GS1 ships in v1, labeled honestly.** GS Paper I is included at launch and described as what it is (currently one full-length paper, more added as questions pass human verification) rather than held back — a UPSC Prelims product without GS1 would feel incomplete. CSAT carries the retake variety in the meantime; GS1's bank grows through the curated pipeline (#25). |

---

## 13. Open Decisions

All three decisions this section tracked were **resolved by the owner on 31 Aug 2026** — see decision log entries #26 (wrong keys: recompute + notify), #27 (public claim: unlimited retakes from a growing verified bank, no paper count), and #28 (GS1 ships in v1, labeled honestly). The "1,000 papers" wording sweep across code and UI (seed titles/descriptions, course pages, admin copy, one e2e assertion) is unblocked by #27 and handled in the same change that records these decisions.

The only decision still open in the log is **#24** (certificates under the mock-test model).
