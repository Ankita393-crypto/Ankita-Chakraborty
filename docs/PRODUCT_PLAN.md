...
| 17 | AI study chapters from weakness report | **Decided (owner)** | Each weak subject in the report is a hyperlink opening a new tab (`/learn/<subject>`) with a UPSC-coursebook-style chapter generated on demand by AI ("the world is the knowledgebase"), cached per subject+language, with a verify-independently disclaimer. Requires the OpenAI API key; until then the page shows verified seeded courses covering that subject. Mock price placeholder ₹299/attempt (owner to confirm). |
