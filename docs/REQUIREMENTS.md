...
| ID | Requirement | Priority |
|---|---|---|
| FR-19 | A learner can purchase a **mock-test bunch**: a flat **₹249 one-time** fee unlocks a full mock series with **unlimited attempts**, papers composed from a growing verified question bank — no fixed paper-count promise (product plan Decision #25; final claim wording open, #27). Starter scope: **UPSC Prelims (GS Paper I + CSAT) and UPSC Mains**, in **English and Hindi only** (product plan Decision #23). There is no per-course entrance exam and no pass/fail gate on any free course. | M |
...
| FR-22 | Mock papers are composed deterministically from a per-bunch verified question bank ("permutation and combination" — product plan Decision #18), replicating the real UPSC paper's format, timing, and marking scheme (not a short quiz). While the bank is small enough that papers meaningfully overlap, the UI discloses that papers share questions. CSAT bank growth may use the AI pipeline of Decision #25 (pre-generated pool, automated verification, review queue — never live at attempt time); GS1 bank growth is curated-only. **No proctoring** (decided [OQ-4]). | M |
...
**Still open after the latest owner review:** the free-tier daily generation cap [OQ-7] (premium tier itself is now **parked**, not merely deferred — product plan Decision #18, so FR-29/FR-30 are inactive until the owner revisits it); the Mains regional-language list (product plan Decision #23); and mock-attempt certificates (product plan Decision #24). Resolved on 31 Aug 2026: wrong-answer-key remediation is **recompute + notify** (Decision #26); the public claim is **"unlimited retakes from a growing bank of verified questions"** with no fixed paper count (Decision #27); and **GS Paper I ships in v1 with honest single-paper labelling** (Decision #28). Operational prerequisites that remain are tasks, not decisions: domain/trademark check for "Bodhi", payment gateway individual onboarding, YouTube channel creation, and Privacy Policy / Terms of Service drafting.
